...
export default class Shop extends AddressComponent{
	...
	async getRestaurants(req, res, next) {
		...
		if (restaurant_category_ids.length && Number(restaurant_category_ids[0])) {
			const categoryFilter = await category.findById(restaurant_category_ids[0]);
			Object.assign(filter, { category: categoryFilter })
		}
		//按照距离，评分，销量等排序
		let sortBy = {};
		if (Number(order_by)) {
		...
		let to = '';
		//获取百度地图测局所需经度纬度(to:  31.11,124.11|31.11,124.11|31.11,124.11|31.11,124.11|31.11,124.11)
		...
		try {
			if (restaurants.length) {
				...
				restaurants.map((item, index) => {
					return Object.assign(item, distance_duration[index]) // 返回 distance&&durationtime 
				})
			}
		} catch (err) {
			console.log('从addressComoponent获取测距数据失败', err); // 就算失败了 也有默认的东西 
			restaurants.map((item, index) => {
