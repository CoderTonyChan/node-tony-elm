...
export default class CityHandle extends AddressComponent {
    constructor() {
        super()
        this.getCity = this.getCity.bind(this); // 为啥要绑定这个呢? 应该是这个里面也用到了this
        this.getExactAddress = this.getExactAddress.bind(this);
        this.pois = this.pois.bind(this);
    }
    async getCity(req, res, next) { // 要绑定这个 是因为这个有可能被window直接调用 this指向不对????  
